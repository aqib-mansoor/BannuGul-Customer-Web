  ...
  ${isOpen ? "translate-x-0" : "translate-x-full"}`}>
                  ...
                  {cartItems.map(item => (
                    <div key={item.product_id} className="flex items-center justify-between p-3 mb-3 border rounded-lg shadow-sm hover:shadow-md transition">
                      <div className="flex items-center gap-3">
                        <img src={item.product?.thumb ? `${import.meta.env.VITE_BASE_URL}/storage/${item.product.thumb}` : "/placeholder.png"} alt={item.product?.name} className="w-14 h-14 object-cover rounded-lg border" />
                        <div>
                          <span className="font-medium">{item.product?.name}</span>
                          <div className="text-sm text-gray-500">{CURRENCY} {item.product?.price}</div>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 bg-green-100 px-2 py-1 rounded-full">
                        <button onClick={() => handleRemove(item)} className="p-1 hover:bg-green-200 rounded-full"><Minus size={14} /></button>
                        <span className="text-green-600 font-medium">{item.quantity}</span>
                        <button onClick={() => handleAdd(item)} className="p-1 hover:bg-green-200 rounded-full"><Plus size={14} /></button>
                        <button onClick={() => handleRemoveItem(item.product_id)} className="p-1 text-red-600 hover:bg-red-100 rounded-full"><Trash2 size={16} /></button>
                      </div>
                    </div>
                  ))}
                  <button
        ...
        {step === 1 && cartItems.length > 0 && (
          <div className="p-4 border-t bg-gray-50 space-y-2">
            <button onClick={handleEmptyCart} className="w-full bg-red-500 text-white py-2 rounded-lg hover:bg-red-600 flex items-center justify-center gap-2"><Trash2 size={18} /> Clear Cart</button>
            <button onClick={() => setStep(2)} className="w-full bg-green-600 text-white py-2 rounded-lg hover:bg-green-700 transition">Checkout</button>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="p-4 border-t bg-gray-50 flex justify-between">
            ...
            <button onClick={handlePlaceOrder} className="px-4 py-2 bg-green-600 text-white rounded-lg">Place Order</button>
          </div>
        )}
